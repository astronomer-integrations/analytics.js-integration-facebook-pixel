...
FacebookPixel.prototype.initialize = function() {
  ...
  window.fbq.queue = [];
  fbPixelId = this.options.pixelId;
...
FacebookPixel.prototype.track = function(track) {
  ...
  if (![].concat(standard, legacy).length) {
    superTrack('trackCustom', event, payload, {
  ...
  each(function(event) {
    ...
    superTrack('track', event, payload, {
      eventID: track.proxy('messageId')
    });
  }, standard);
...
FacebookPixel.prototype.buildPayload = function(track, isStandardEvent) {
  ...
  for (var key in blacklistPiiProperties) {
    customPiiProperties[key] = blacklistPiiProperties[key] == "Hash";
  }
