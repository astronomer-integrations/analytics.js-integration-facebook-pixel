...
var superTrack = function(type, pixelId, eventName, customData, isSingle) {
  ...
  var sendTrackSingleCustom = function() {
    if (customData) {
      window.fbq('trackSingleCustom', pixelId, eventName, customData);
    } else {
      window.fbq('trackSingleCustom', pixelId, eventName);
    }
  };
  if (isSingle) {
    if (type === 'track') {
