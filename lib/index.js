...
FacebookPixel.prototype.page = function() {
  window.fbq(this.options.onlyTrackSingle ? 'trackSingle' : 'track', 'PageView');
};
...
FacebookPixel.prototype.track = function(track) {
  ...
  if (![].concat(standard, legacy).length) {
    window.fbq(this.options.onlyTrackSingle ? 'trackSingleCustom' : 'trackCustom', event, payload);
    return;
  }
  ...
  each(function(event) {
    ...
    window.fbq(this.options.onlyTrackSingle ? 'trackSingle' : 'track', event, payload);
  }, standard);

  // legacy conversion events — mapped to specific "pixelId"s
  // send only currency and value
  each(function(event) {
    window.fbq(this.options.onlyTrackSingle ? 'trackSingle' : 'track', event, {
...
FacebookPixel.prototype.productListViewed = function(track) {
  window.fbq(this.options.onlyTrackSingle ? 'trackSingle' : 'track', 'ViewContent', {
...
FacebookPixel.prototype.productAdded = function(track) {
  window.fbq(this.options.onlyTrackSingle ? 'trackSingle' : 'track', 'AddToCart', {
    content_ids: [track.productId() || track.id() || track.sku() || ''],
    content_type: 'product',
    content_name: track.name() || '',
    content_category: track.category() || '',
...
FacebookPixel.prototype.orderCompleted = function(track) {
  ...
  window.fbq(this.options.onlyTrackSingle ? 'trackSingle' : 'track', 'Purchase', {
