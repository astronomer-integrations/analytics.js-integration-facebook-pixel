...
var _track = 'track';//(this.options.onlyTrackSingle ? 'trackSingle' : 'track');
var _trackCustom = 'trackCustom';//(this.options.onlyTrackSingle ? 'trackSingleCustom' : 'trackCustom');
...
FacebookPixel.prototype.track = function(track) {
  ...
  if (![].concat(standard, legacy).length) {
    window.fbq(_trackCustom, event, payload);
    return;
  }
